...
// GetSampleCount aggregates the sample count of each inner Historgram.
func (vec HistogramVec) GetSampleCount() uint64 {
	var count uint64
...
// GetSampleSum aggregates the sample sum of each inner Historgram.
func (vec HistogramVec) GetSampleSum() float64 {
	var sum float64
	for _, hist := range vec {
...
func (vec HistogramVec) Quantile(q float64) float64 {
	...
	for i, hist := range vec {
		for _, bckt := range hist.Bucket {
			if i == 0 {
				...
				buckets[i].count += float64(bckt.GetCumulativeCount())
			}
		}
	}
	...
	if len(buckets) == 0 || buckets[len(buckets)-1].upperBound != math.Inf(+1) {
		...
		buckets = append(buckets, bucket{
			count:      float64(vec.GetSampleCount()),
