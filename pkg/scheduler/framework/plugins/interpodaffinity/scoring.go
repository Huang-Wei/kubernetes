...
func (pl *InterPodAffinity) PreScore(
	...
	if pl.sharedLister == nil {
		return framework.AsStatus(Name, fmt.Errorf("InterPodAffinity PreScore with empty shared lister found"))
	}
	...
	if hasPreferredAffinityConstraints || hasPreferredAntiAffinityConstraints {
		...
		if err != nil {
			framework.AsStatus(Name, fmt.Errorf("get all nodes from shared lister error, err: %v", err))
		}
	} else {
		allNodes, err = pl.sharedLister.NodeInfos().HavePodsWithAffinityList()
		if err != nil {
			framework.AsStatus(Name, fmt.Errorf("get pods with affinity list error, err: %v", err))
		}
	}
	...
	if podInfo.ParseError != nil {
		...
		return framework.AsStatus(Name, fmt.Errorf("parsing pod: %+v", podInfo.ParseError))
	}
...
func (pl *InterPodAffinity) Score(ctx context.Context, cycleState *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	if err != nil || nodeInfo.Node() == nil {
		return 0, framework.AsStatus(Name, fmt.Errorf("getting node %q from Snapshot: %v, node is nil: %v", nodeName, err, nodeInfo.Node() == nil))
	}
	node := nodeInfo.Node()
