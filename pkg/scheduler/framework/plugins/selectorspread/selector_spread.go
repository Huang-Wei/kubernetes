...
func (pl *SelectorSpread) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
	...
	if err != nil {
		return 0, framework.AsStatus(Name, fmt.Errorf("reading %q from cycleState: %w", preScoreStateKey, err))
	}

	s, ok := c.(*preScoreState)
	if !ok {
		return 0, framework.AsStatus(Name, fmt.Errorf("cannot convert saved state to tainttoleration.preScoreState"))
	}
...
func (pl *SelectorSpread) NormalizeScore(ctx context.Context, state *framework.CycleState, pod *v1.Pod, scores framework.NodeScoreList) *framework.Status {
	...
	for i := range scores {
		...
		if err != nil {
			return framework.AsStatus(Name, fmt.Errorf("getting node %q from Snapshot: %v", scores[i].Name, err))
		}
