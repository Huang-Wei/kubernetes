	...
	tests := []struct {
	...
	}{
		{
			...
			wantStatus: framework.NewStatus(framework.UnschedulableAndUnresolvable,
				framework.NewFailure(Name, "node(s) had taint {dedicated: user1}, that the pod didn't tolerate")),
		},
		{
		...
		},
		{
			...
			node: nodeWithTaints("nodeA", []v1.Taint{{Key: "foo", Value: "bar", Effect: "NoSchedule"}}),
			wantStatus: framework.NewStatus(framework.UnschedulableAndUnresolvable,
				framework.NewFailure(Name, "node(s) had taint {foo: bar}, that the pod didn't tolerate")),
		},
		{
