...
// Tests the behavior of TaintBasedEvictions. Following scenarios are included:
...
var _ = SIGDescribe("TaintBasedEvictions Single Pod [Serial]", func() {
	var cs clientset.Interface
	var ns string
	f := framework.NewDefaultFramework("tbe-single-pod")
...
var _ = SIGDescribe("TaintBasedEvictions Multiple Pods [Serial]", func() {
	var cs clientset.Interface
	var ns string
	f := framework.NewDefaultFramework("tbe-multiple-pods")
	...
	It("evicts pods from tainted nodes", func() {
		...
		case podName := <-observedDeletions:
			if podName == podGroup+"1" {
				framework.Logf("Noticed Pod %q gets evicted. Test successful", podName)
