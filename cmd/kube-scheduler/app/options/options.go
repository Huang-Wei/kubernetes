...
func (o *Options) ApplyDeprecated() {
	...
	// Obtain insecure serving CLI args. If not specified in command line, derive the default settings.
	insecureServing := o.Flags.FlagSet("insecure serving")
	if !insecureServing.Changed("address") {
		o.CombinedInsecureServing.BindAddress = "0.0.0.0"
	}
	if !insecureServing.Changed("port") {
		o.CombinedInsecureServing.BindPort = kubeschedulerconfig.DefaultInsecureSchedulerPort
	}
